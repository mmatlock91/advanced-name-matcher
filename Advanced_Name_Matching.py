...
def clean_text(text):
    ...
    company_suffixes_end_of_string = [
        'inc', 'corp', 'co', 'ltd', 'p.c.', 'pc', 'llc', 'l.l.c.', 'l.c.', 'lc',
        'ltd. co.', 'ltd co', 'lp', 'l.p.', 'llp', 'l.l.l.p.', 'pllc', 'b.v.', 'gmbh',
        'enterprises', 'industries', 'solutions', 'ventures', 'holdings', 'group',
        'services', 'technology', 'systems',
        'inc.', 'corp.', 'co.', 'ltd.', 'pc.', 'llc.', 'l.l.c.', 'l.c.', 'lc.',
        'lp.', 'l.p.', 'llp.', 'p.l.l.c.', '& co.'
    ]
    ...
    abbreviations = {
        r'\btech\b': 'technology', r'\bsvcs?\b': 'services', r'\bsolns?\b': 'solutions',
        r'\bmktg\b': 'marketing', r'\bmgmt\b': 'management', r'\bdev\b': 'development',
        r'\bintl\b': 'international', r'\bcorp\b': 'corporation', r'\binc\b': 'incorporated'
    }
...
def jaccard_similarity(text1, text2):
    """Calculates Jaccard similarity between two texts based on their words."""
...
def ultimate_validation(text1, text2, jaccard_threshold):
    """Applies a more robust, multi-stage validation to act as a final filter for AI suggestions."""
    ...
    if long_str.startswith(short_str) and fuzz.WRatio(short_str, long_str) >= 90:
        return True
...
def process_fuzzy_market_chunk(args):
    """Worker function for the initial high-confidence fuzzy matching pass."""
...
def pool_initializer_semantic(model_name, cross_encoder_model_name):
    """Initializes the heavy Sentence Transformer models in each child process."""
...
def process_semantic_market_chunk(args):
    """Worker function for the hybrid (FAISS + Cross-Encoder Reranking) matching pass."""
    global RETRIEVER_MODEL, CROSS_ENCODER
...
class Worker(QObject):
    ...
    def _create_summary_report(self, all_matches):
        """Generates the specific summary report file based on the master data."""
        self.log.emit("Creating summary report...")
        master_name_col, market_col = 'Master Name', 'Market'
        
        master_to_source_map = {}
        for (source_name, market), match_details in all_matches.items():
            master_name = match_details.get('master')
            if not master_name: continue
            key = (master_name, market)
            if key not in master_to_source_map: master_to_source_map[key] = []
            master_to_source_map[key].append({'source': source_name, 'method': match_details.get('method', 'N/A')})

        unique_master_df = self.df_master_source.drop_duplicates(subset=[master_name_col, market_col]).copy()
        
        def get_match_info(row):
            key = (row[master_name_col], row[market_col])
            if key in master_to_source_map:
                match_list = master_to_source_map[key]
                matched_sources = ", ".join(sorted(list(set(d['source'] for d in match_list))))
                match_methods = ", ".join(sorted(list(set(d['method'] for d in match_list))))
                return "Matched", matched_sources, match_methods
            return "Unmatched", None, None

        unique_master_df[['Match Status', 'Matched Name', 'Match Method']] = \
            unique_master_df.apply(get_match_info, axis=1, result_type='expand')
        unique_master_df.rename(columns={master_name_col: 'Master Name'}, inplace=True)

        final_cols_ordered = ['Match Status', 'Master Name', 'Matched Name', 'Match Method', 'Master Unique ID', 'Region', 'Market']
        for col in final_cols_ordered:
            if col not in unique_master_df.columns: unique_master_df[col] = ''
        
        self.summary_df = unique_master_df[final_cols_ordered]
        self.log.emit("Summary report created successfully.")

    def run(self):
        try:
            start_total_time = time.time()
            source_name_col, market_col = 'Source Name', 'Market'
            master_name_col = 'Master Name'
            ...
            if 'Merchant Name' in self.df_to_update.columns: self.df_to_update.rename(columns={'Merchant Name': source_name_col}, inplace=True)
            if 'Local Name' in self.df_master_source.columns: self.df_master_source.rename(columns={'Local Name': master_name_col}, inplace=True)
            ...
            df_to_process[source_name_col] = df_to_process[source_name_col].astype(str)
            df_to_process[market_col] = df_to_process[market_col].astype(str).str.strip().str.upper()
            self.df_master_source[master_name_col] = self.df_master_source[master_name_col].astype(str)
            self.df_master_source[market_col] = self.df_master_source[market_col].astype(str).str.strip().str.upper()

            source_grouped = df_to_process.groupby(market_col)
            master_grouped = self.df_master_source.groupby(market_col)
            all_matches = {}

            # --- Pass 1: RapidFuzz High-Confidence Match ---
            fuzzy_tasks = []
            for market, group in source_grouped:
                if market in master_grouped.groups:
                    master_names = master_grouped.get_group(market)[master_name_col].dropna().unique().tolist()
                    source_names = group[source_name_col].dropna().unique().tolist()
            ...
            # --- Pass 2: Semantic AI on Unmatched Names ---
            self.log.emit("\n--- Pass 2: Running Semantic AI on Unmatched Names ---")
            df_to_process['temp_key'] = list(zip(df_to_process[source_name_col], df_to_process[market_col]))
            unmatched_df = df_to_process[~df_to_process['temp_key'].isin(all_matches.keys())]

            semantic_tasks = []
            if not unmatched_df.empty:
                for market, group in unmatched_df.groupby(market_col):
                     if market in master_grouped.groups:
                        master_names = master_grouped.get_group(market)[master_name_col].dropna().unique().tolist()
                        source_names = group[source_name_col].dropna().unique().tolist()
                        if source_names and master_names:
            ...
            # 1. Map 'Matched Name' back to the dataframe to be processed
            ...
            # 2. Recombine with already-matched rows to get the full source dataframe with matches
            if not df_already_matched.empty:
                final_source_df = pd.concat([df_already_matched, df_to_process], ignore_index=True)
            else:
                final_source_df = df_to_process

            # 3. Perform a LEFT JOIN to bring in ALL data from the master file
            master_for_join = self.df_master_source.drop_duplicates(subset=[master_name_col, market_col])
            self.df_to_update = pd.merge(
                final_source_df,
                master_for_join,
                how='left',
                left_on=[matched_name_col, market_col],
                right_on=[master_name_col, market_col]
            )
            self.log.emit("Join complete. Final output file created.")
            
            # 4. Create the specific summary report
            self._create_summary_report(all_matches)
            
            # 5. Clean both dataframes for final output
    ...
    def save_files(self, save_option, file_format):
        try:
            ...
            if save_option == 'quit':
                self.finished_saving.emit(None, 'quit')
                return

            df_to_save, suffix = None, ""
            if save_option == 'main':
                df_to_save, suffix = self.df_to_update, "_matched"
            elif save_option == 'summary_report':
                df_to_save, suffix = self.summary_df, "_summary_report"
            
            if df_to_save is not None and not df_to_save.empty:
                output_path = os.path.join(self.output_dir, f"{base_name}{suffix}.{file_format}")
                self.log.emit(f"\nSaving '{os.path.basename(output_path)}'...")
                if file_format == "xlsx":
                    df_to_save.to_excel(output_path, index=False, na_rep='')
                else:
                    df_to_save.to_csv(output_path, index=False, na_rep='')
                self.log.emit("Save successful!")
            elif df_to_save is not None:
                 self.log.emit(f"\nSkipping save for '{save_option}' because it is empty.")

            self.finished_saving.emit(self.output_dir, save_option)
        except Exception as e:
            self.error.emit(f"An error occurred during saving: {e}")
...
class MatcherApp(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(0, 0, 800, 800); self.center_window()
        ...
        self.setStyleSheet("""
            ...
            QLineEdit,QTextEdit,QSpinBox,QDoubleSpinBox { background-color:#3c3c3c; border:1px solid #555; border-radius:4px; padding:4px }
        ...
        layout = QGridLayout(central_widget); layout.setContentsMargins(15, 15, 15, 15); layout.setSpacing(10)
        ...
        layout.addWidget(QLabel("Source Data File:", objectName="title_label"), 0, 0); layout.addWidget(self.source_file_entry, 0, 1); layout.addWidget(self.browse_source_btn, 0, 2)
        layout.addWidget(QLabel("Master Reference File:", objectName="title_label"), 1, 0); layout.addWidget(self.master_file_entry, 1, 1); layout.addWidget(self.browse_master_btn, 1, 2)
        layout.addWidget(QLabel("Output Folder:", objectName="title_label"), 2, 0); layout.addWidget(self.output_dir_entry, 2, 1); layout.addWidget(self.browse_output_dir_btn, 2, 2)
        ...
        options_layout.addWidget(QLabel("<b>Pass 1: Fuzzy Threshold:</b>", objectName="header_label"), 1, 0); options_layout.addWidget(self.fuzz_slider, 1, 1); options_layout.addWidget(self.fuzz_threshold_spinbox, 1, 2)
        ...
        options_layout.addWidget(QLabel("<b>Pass 2: AI Threshold:</b>", objectName="header_label"), 2, 0); options_layout.addWidget(self.semantic_slider, 2, 1); options_layout.addWidget(self.semantic_threshold_spinbox, 2, 2)
        layout.addWidget(options_group, 3, 0, 1, 3)
        ...
        hybrid_layout.addWidget(QLabel("<b>Semantic Weight:</b>"), 0, 0); hybrid_layout.addWidget(self.semantic_weight_slider, 0, 1); hybrid_layout.addWidget(self.semantic_weight_spinbox, 0, 2)
        ...
        hybrid_layout.addWidget(QLabel("<b>Fuzzy Weight:</b>"), 1, 0); hybrid_layout.addWidget(self.fuzzy_weight_slider, 1, 1); hybrid_layout.addWidget(self.fuzzy_weight_spinbox, 1, 2)
        ...
        hybrid_layout.addWidget(QLabel("<b>Jaccard Filter:</b>"), 2, 0); hybrid_layout.addWidget(self.jaccard_thresh_slider, 2, 1); hybrid_layout.addWidget(self.jaccard_thresh_spinbox, 2, 2)
        layout.addWidget(hybrid_group, 4, 0, 1, 3)
        
        perf_group = QGroupBox("Performance Tuning (AI Pass)"); perf_layout = QGridLayout(perf_group)
        self.k_spinbox = QSpinBox(); self.k_spinbox.setRange(3, 20); self.k_spinbox.setValue(10)
        perf_layout.addWidget(QLabel("Candidates to Check (K):"), 0, 0, Qt.AlignLeft); perf_layout.addWidget(self.k_spinbox, 0, 1, Qt.AlignLeft); perf_layout.setColumnStretch(2, 1)
        layout.addWidget(perf_group, 5, 0, 1, 3)
        
        self.unmatched_only_checkbox = QCheckBox("Process unmatched merchants only (requires 'Matched Name' column)")
        self.run_button = QPushButton("Run Matching"); self.run_button.setDisabled(True)
        self.show_save_dialog_btn = QPushButton("Show Save Options"); self.show_save_dialog_btn.hide()
        
        action_button_layout = QHBoxLayout(); action_button_layout.addWidget(self.run_button); action_button_layout.addWidget(self.show_save_dialog_btn)
        run_control_layout = QVBoxLayout(); run_control_layout.addWidget(self.unmatched_only_checkbox, 0, Qt.AlignCenter); run_control_layout.addLayout(action_button_layout)
        layout.addLayout(run_control_layout, 6, 0, 1, 3)
        
        self.progress_label = QLabel("Ready to run."); self.progress_label.setStyleSheet("font-weight:bold; color:#00aaff;")
        layout.addWidget(self.progress_label, 7, 0, 1, 3)
        self.log_textbox = QTextEdit(readOnly=True); layout.addWidget(self.log_textbox, 8, 0, 1, 3)
        
        layout.setRowStretch(8, 1); layout.setColumnStretch(1, 1)
        ...
        self.show_save_dialog_btn.clicked.connect(self.on_prompt_for_saving)
        for w in [self.source_file_entry, self.master_file_entry, self.output_dir_entry]: w.textChanged.connect(self.check_paths)
        
        self.fuzz_slider.valueChanged.connect(lambda v: self.update_spinbox_from_slider(v, self.fuzz_threshold_spinbox))
        self.fuzz_threshold_spinbox.valueChanged.connect(lambda v: self.update_slider_from_spinbox(v, self.fuzz_slider))
        self.semantic_slider.valueChanged.connect(lambda v: self.update_spinbox_from_slider(v, self.semantic_threshold_spinbox))
        self.semantic_threshold_spinbox.valueChanged.connect(lambda v: self.update_slider_from_spinbox(v, self.semantic_slider))
        self.semantic_weight_slider.valueChanged.connect(lambda v: self.update_spinbox_from_slider(v, self.semantic_weight_spinbox))
        self.semantic_weight_spinbox.valueChanged.connect(lambda v: self.update_slider_from_spinbox(v, self.semantic_weight_slider))
        self.fuzzy_weight_slider.valueChanged.connect(lambda v: self.update_spinbox_from_slider(v, self.fuzzy_weight_spinbox))
        self.fuzzy_weight_spinbox.valueChanged.connect(lambda v: self.update_slider_from_spinbox(v, self.fuzzy_weight_slider))
        self.jaccard_thresh_slider.valueChanged.connect(lambda v: self.update_spinbox_from_slider(v, self.jaccard_thresh_spinbox))
        self.jaccard_thresh_spinbox.valueChanged.connect(lambda v: self.update_slider_from_spinbox(v, self.jaccard_thresh_slider))
    ...
    def check_paths(self): self.run_button.setEnabled(all([self.source_file_entry.text(), self.master_file_entry.text(), self.output_dir_entry.text()]))
    def select_source_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "Select Source Data File", "", "Excel/CSV Files (*.xlsx *.xls *.csv)");
        if path: self.source_file_entry.setText(path)
    def select_master_file(self):
        ...
        if path: self.master_file_entry.setText(path)
    def select_output_dir(self):
        path = QFileDialog.getExistingDirectory(self, "Select Output Folder");
        if path: self.output_dir_entry.setText(path)
    def start_processing(self):
        self.log_textbox.clear(); self.run_button.setText("Cancel"); self.run_button.clicked.disconnect(); self.run_button.clicked.connect(self.cancel_processing)
        ...
        self.worker.error.connect(self.on_processing_error); self.worker.log.connect(self.log_textbox.append)
    ...
    def reset_ui_state(self):
        ...
        self.run_button.clicked.connect(self.start_processing); self.check_paths()
    ...
    def closeEvent(self, event):
        if self.thread and self.thread.isRunning():
            if QMessageBox.question(self, 'Confirm Quit', "A matching process is running. Are you sure you want to quit?",
               QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) == QMessageBox.StandardButton.Yes:
                self.cancel_processing(); event.accept()
            else: event.ignore()
        else: event.accept()
